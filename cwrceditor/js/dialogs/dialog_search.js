...
//			    '</div>'+
	...
	var doQuery = function() {
		...
		if (lookupService == 'lookup_project' && query != '') {
			...
			$.ajax({
				url: w.baseUrl+"/solr/solr_core_default/select",
				data: {
					//wt=json&start="+start+"&rows="+rows+"&q=" + query,
					'wt': 'json',
					'start': 0,
					'rows': 100,
					'fq': 'gen.type='+fq,
		...
		show: function(config) {
			...
			search.dialog('option', 'buttons', [{
				...
				click: function() {
					...
					var dialogCallback = function(id) {
						// alert(id);
						// $('#search_query').val(id);
					...
					switch(config.title) {
						case "Person":
							writer.cwrcDialogs.newPersonDialog(dialogCallback);	
						break;
						case "Organization":
							writer.cwrcDialogs.newOrganizationDialog(dialogCallback);	
						break;							
						default:
							createNew();
						break;
